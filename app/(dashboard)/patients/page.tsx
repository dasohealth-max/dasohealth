...
function LocationMapPicker({ lat, lng, onSelect }: {
  ...
  onSelect: (d: { district: string; region: string; lat: number; lng: number; cityName: string }) => void;
}) {
  ...
  useEffect(() => {
    ...
    import('leaflet').then((L) => {
      ...
      map.on('click', (e: LeafletAny) => {
        ...
        onSelect({ district: nearest.district, region: nearest.region, lat: clat, lng: clng, cityName: nearest.name });
      });
  ...
  function pickCity(city: typeof SOMALIA_CITIES[0]) {
    setSearch(city.name);
    setShowSug(false);
    onSelect({ district: city.district, region: city.region, lat: city.lat, lng: city.lng, cityName: city.name });
  }
...
function PatientFullForm({
  ...
  function handleMapSelect(d: { district: string; region: string; lat: number; lng: number; cityName: string }) {
