...
import { fromPrisma, getAllTransportJobs as fetchAllTransportJobs } from '@/lib/api/transport';
...
export async function getAllTransportJobs(): Promise<TransportJob[]> {
  const denied = await guard('transport', 'view');
  if (denied) throw new Error(denied.error);
  return fetchAllTransportJobs();
}
